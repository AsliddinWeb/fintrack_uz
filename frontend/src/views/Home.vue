<template>
    <div class="chartt">
      <Header />
      <div class="relative">
        <div class="relative max-w-7xl mx-auto px-2 pt-6 sm:px-6 lg:px-4 lg:pt-20">
          <div class="mx-auto grid max-w-[40rem] grid-cols-1 gap-x-8 gap-y-16 lg:max-w-none lg:grid-cols-3">
            <div class="flex flex-col lg:pb-6">
              <h1 class="mt-4 text-4xl font-extrabold leading-none tracking-tight text-slate-900 sm:text-5xl sm:leading-[3.5rem]">
                Finance Tracker
              </h1>
              <div class="order-first flex items-center gap-2">
                <p class="text-base font-semibold leading-7 text-slate-500">Kirim-chiqim hisoboti</p>
              </div>
              <p class="mt-6 text-base leading-7 text-slate-700">
                FinTrack Uz sizning xarajatlaringizni, daromadlaringizni, qarzlaringizni va boshqa moliyaviy ma'lumotlaringizni bir joyda saqlash va ularni samarali boshqarish imkonini beradi.
              </p>
              ...
              <div class="mt-10 flex flex-wrap gap-4">
                <a href="https://catalyst.tailwindui.com" class="inline-flex justify-center rounded-lg text-sm font-semibold py-3 px-4 bg-slate-900 text-white hover:bg-slate-700" target="_blank">
                  <span><span class="inline-flex items-center whitespace-nowrap"><span>Qarzdorlik</span>
                ...
                <a href="https://catalyst.tailwindui.com/docs" class="inline-flex justify-center rounded-lg text-sm font-semibold py-3 px-4 bg-white/0 text-slate-900 ring-1 ring-slate-900/10 hover:bg-white/25 hover:ring-slate-900/15 whitespace-nowrap" target="_blank">
                  <span>Xarajatlar<span aria-hidden="true" class="text-black/25">→</span></span></a>
              </div>
            </div>
            <div class="relative lg:col-span-2 border-2 rounded-md p-4">
              <div class="z-0 hidden md:block">
                ...
                <FinanceChart v-if="chartData" :chartData="chartData" :chartOptions="chartOptions" />
              </div>
            </div>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      async fetchChartData(period) {
        try {
          const response = await axios.get(`https://your-backend-api-url/finance-data?period=${period}`);
          if (response.data) {
            const financeData = response.data;
            console.log(financeData.labels); // Ma'lumotlarni konsolda ko'rsatish
